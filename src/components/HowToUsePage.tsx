...
const GuideSection: React.FC<{
    ...
    <Card className="animate-fade-in border-border/40 bg-card/50 backdrop-blur-sm transition-colors duration-300">
        <CardHeader className="flex flex-col md:flex-row items-start gap-4 space-y-0 pb-4">
            <div className="bg-primary/10 border border-primary/20 rounded-xl p-3 flex-shrink-0 shadow-[0_0_20px_rgba(72,168,163,0.15)]">
                {icon}
            </div>
            <div>
        ...
        <CardContent className="space-y-4 pl-4 md:pl-20 text-muted-foreground text-sm leading-relaxed">
            {children}
        </CardContent>
    </Card>
);
...
const CodeBlock: React.FC<{ children: React.ReactNode }> = ({ children }) => (
    <pre className="bg-zinc-950 text-cyan-300 p-4 rounded-lg text-xs font-mono overflow-x-auto border border-primary/10 shadow-inner">
        <code>{children}</code>
    </pre>
);
...
const HowToUsePage: React.FC = () => {
    return (
        <div className="w-full max-w-4xl mx-auto space-y-8 pb-20">
            <header className="text-center space-y-3 animate-fade-in py-6">
                <div className="inline-flex justify-center items-center p-3 bg-primary/10 rounded-full mb-2 border border-primary/20">
                    <BookOpenIcon className="h-10 w-10 text-primary" />
                </div>
                <h1 className="text-3xl font-bold tracking-wide text-foreground drop-shadow-lg">Dashboard Integration Guide</h1>
            ...
            >
                <h4 className="font-bold text-lg text-foreground">Communication Architecture</h4>
                ...
                <h4 className="font-bold text-lg text-foreground pt-4">Webhook Node Setup (The Trigger)</h4>
                <p>
                ...
                <h4 className="font-bold text-lg text-foreground pt-4">Server-Sent Events Node (The Streamer)</h4>
                <p>
                ...
                <p className="text-xs bg-muted/40 p-2 rounded-lg border border-border/40 text-foreground/80">
            ...
            >
                ...
                <p className="pt-2 text-sm">For any issues, please visit the <strong>Support</strong> page for FAQs and contact information.</p>
            </GuideSection>
        </div>
    );
};
