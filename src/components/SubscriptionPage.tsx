                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pt-6">
                    <div>
                        <h1 className="text-3xl md:text-4xl font-bold text-foreground tracking-tight flex items-center gap-3">
                            Client Dashboard <span className="text-sm font-normal text-muted-foreground bg-muted/20 px-3 py-1 rounded-full border border-border/40">v2.5</span>
                        </h1>
                        <p className="text-muted-foreground mt-1">Overview of your subscription, usage, and system health.</p>
                    </div>
                    <div className="flex gap-3">
                         <Button variant="outline" onClick={onNavigateToPricing} className="border-primary/20 hover:bg-primary/5 text-primary">
                             {isFree ? 'Upgrade Plan' : 'Manage Plan'}
                         </Button>
                         <Button onClick={() => setShowRequestModal(true)} className="bg-primary hover:bg-primary/90 shadow-lg shadow-primary/20">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <Card className={cn("relative border-2 shadow-xl flex flex-col justify-between h-full bg-[#020202]", 
                        ...
                        <div className="absolute inset-0 overflow-hidden rounded-xl pointer-events-none">
                             {currentPlan !== 'free' && (
                                <div className={cn("absolute top-0 right-0 w-64 h-64 rounded-full blur-[80px] opacity-20", 
                                    currentPlan === 'enterprise' ? "bg-purple-600" : "bg-primary"
                                )}></div>
                            )}
                        </div>

                        <CardContent className="p-8 pt-8 relative z-10 h-full flex flex-col">
                            <div className="flex justify-between items-start mb-8">
                                <div className="p-3 bg-white/5 rounded-2xl border border-white/10 backdrop-blur-sm">
                                    <CreditCardIcon className={cn("h-8 w-8", 
                                         currentPlan === 'enterprise' ? "text-purple-400" : 
                                         currentPlan === 'pro' ? "text-primary" : "text-muted-foreground"
                                    )} />
                                </div>
                                <span className={cn("px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider border backdrop-blur-sm",
                            ...
                            <div className="mt-auto">
                                <div className="flex items-center mb-2">
                                    <p className="text-sm text-muted-foreground font-medium uppercase tracking-wide">Current Plan</p>
                                    <InfoTooltip content="Your active subscription tier, pricing, and next renewal date." />
                                </div>
                                <h2 className="text-4xl font-bold tracking-tight text-white mb-1">{details.name}</h2>
                                <div className="flex items-baseline gap-2">
                                    <p className="text-2xl text-white/90 font-medium">{details.priceString}</p>
                                    {!isFree && <span className="text-sm text-muted-foreground">billed monthly</span>}
                                </div>
                                {!isFree && (
                                    <div className="mt-6 pt-4 border-t border-white/10 flex items-center gap-2 text-xs text-muted-foreground">
                    ...
                    <Card className="border-border/40 bg-card/50 backdrop-blur-sm">
                         <CardHeader className="pb-2 border-b border-border/10">
                            <CardTitle className="text-sm font-medium text-muted-foreground uppercase tracking-wider flex items-center">
                                <ActivityIcon className="h-4 w-4 mr-2" /> Support Usage
                                <InfoTooltip content="Tracks your weekly support requests against your plan limit." />
                            </CardTitle>
                         </CardHeader>
                         <CardContent className="p-8 flex items-center justify-between h-[calc(100%-57px)]">
                            <div className="space-y-2">
                                <p className="text-4xl font-bold text-foreground">{usageCount} <span className="text-xl text-muted-foreground font-normal">/ {usageLimit === 'Unlimited' ? '∞' : usageLimit}</span></p>
                                <p className="text-sm text-muted-foreground font-medium">Requests this week</p>
                                <div className="pt-2 flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-primary">
                    ...
                    <Card className="border-border/40 bg-card/50 backdrop-blur-sm">
                         ...
                         <CardContent className="p-8 flex flex-col justify-center h-[calc(100%-57px)]">
                            {isFree ? (
                                <div className="text-center py-2">
                                    <div className="inline-flex p-4 rounded-full bg-muted/30 text-muted-foreground mb-4 border border-border/50">
                                        <LockClosedIcon className="h-8 w-8" />
                                    </div>
                                    <p className="text-base font-bold text-foreground">Credits Locked</p>
                                    <p className="text-sm text-muted-foreground mt-2">Upgrade to Pro for 3 free hours/mo</p>
                                </div>
                            ) : (
                                <div className="space-y-6">
                                    <div className="flex justify-between items-baseline">
                                        <span className="text-4xl font-bold text-purple-500">{details.freeFixesHours} <span className="text-lg text-foreground font-normal">hrs</span></span>
                                        <span className="text-xs font-bold uppercase text-muted-foreground tracking-wider">Remaining</span>
                                    </div>
                                    <div className="space-y-2">
                                        <div className="h-3 w-full bg-muted rounded-full overflow-hidden">
                                            <div className="h-full w-full bg-purple-500 rounded-full shadow-[0_0_10px_rgba(168,85,247,0.5)]"></div>
                                        </div>
                                    ...
                                    <p className="text-sm text-muted-foreground leading-relaxed">Use for minor tweaks, adjustments, & fixes.</p>
                                </div>
                            )}
                         </CardContent>
                    </Card>
                        ...
                        <Card className="border-border/40 bg-card/50 backdrop-blur-sm flex flex-col">
                             <CardHeader className="pb-4 bg-muted/5 border-b border-border/10">
                                <CardTitle className="text-base font-bold flex items-center">
                                    <ShieldCheckIcon className="h-4 w-4 text-green-500 mr-2" /> System Status
                                 ...
                                 <div className="divide-y divide-border/10">
                                    <div className="flex justify-between items-center p-5">
                                        <div className="flex items-center gap-3">
                                            <div className="p-1.5 rounded bg-green-500/10 text-green-500"><ActivityIcon className="h-4 w-4" /></div>
                                            <span className="text-sm font-medium">Uptime Monitor</span>
                                        </div>
                                        {details.uptimeMonitoring ? (
                                            <span className="text-xs font-bold text-green-400 bg-green-500/5 px-2.5 py-1 rounded border border-green-500/10 shadow-sm">99.99% Online</span>
                                        ) : (
                                    ...
                                    <div className="flex justify-between items-center p-5">
                                        <div className="flex items-center gap-3">
                                            <div className="p-1.5 rounded bg-blue-500/10 text-blue-500"><CalendarIcon className="h-4 w-4" /></div>
                                            <span className="text-sm font-medium">Last Health Check</span>
                                        </div>
                                         {details.healthCheck ? (
                                            <span className="text-xs font-mono text-foreground font-medium">{healthCheckTime}</span>
                                        ) : (
                                    ...
                                    <div className="flex justify-between items-center p-5">
                                        <div className="flex items-center gap-3">
                                            <div className="p-1.5 rounded bg-purple-500/10 text-purple-500"><ShieldCheckIcon className="h-4 w-4" /></div>
                         ...
                         <Card className="border-border/40 bg-card/50 backdrop-blur-sm flex flex-col">
                             <CardHeader className="pb-4 bg-muted/5 border-b border-border/10">
                                <CardTitle className="text-base font-bold flex items-center">
                                    <StarIcon className="h-4 w-4 text-yellow-500 mr-2" /> Plan Capabilities
                             ...
                             <CardContent className="p-0 flex-grow">
                                  <div className="divide-y divide-border/10">
                                     <div className="p-5 flex justify-between items-center">
                                         <span className="text-sm text-muted-foreground">AI Assistant Level</span>
                                         <span className="text-sm font-semibold text-foreground">{details.aiCapability}</span>
                                     </div>
                                     <div className="p-5 flex justify-between items-center">
                                         <span className="text-sm text-muted-foreground">SLA Response Time</span>
                                         <span className="text-sm font-semibold text-foreground">{details.responseTimeBusiness}</span>
                                     </div>
                                     <div className="p-5 flex justify-between items-center">
                                         <span className="text-sm text-muted-foreground">Priority Queue</span>
                                         <span className={`text-xs font-bold px-2 py-1 rounded uppercase tracking-wide ${
                                             details.priority === 'Highest' ? 'bg-purple-500/10 text-purple-400 border border-purple-500/20' : 
                    ...
                    {/* 5. Add-ons & Extras (Col-span-1) - Centered Content */}
                     <Card className="border-border/40 bg-card/50 backdrop-blur-sm flex flex-col">
                        ...
                        <CardContent className="p-8 flex flex-col items-center justify-center flex-grow gap-6">
                             {has247Addon || details.true247Support ? (
                                 <div className="w-full p-6 rounded-2xl bg-green-500/5 border border-green-500/20 flex flex-col items-center text-center gap-3 shadow-[0_0_20px_-5px_rgba(34,197,94,0.1)]">
                                     <div className="p-3 bg-green-500/10 rounded-full text-green-500 mb-1"><CheckBadgeIcon className="h-8 w-8"/></div>
                                     <div>
                                         <p className="font-bold text-green-400 text-lg">24/7 Support Active</p>
                                         <p className="text-sm text-muted-foreground mt-1">Engineering team is on standby.</p>
                                     </div>
                                 </div>
                             ) : (
                                 <div className="w-full p-6 rounded-2xl bg-blue-500/5 border border-blue-500/20 flex flex-col items-center text-center gap-3">
                                     <div className="p-3 bg-blue-500/10 rounded-full text-blue-400 mb-1"><ShieldCheckIcon className="h-8 w-8"/></div>
                                     <div>
                                        <p className="font-bold text-blue-400 text-lg mb-1">24/7 Support Available</p>
                                        <p className="text-sm text-muted-foreground mb-4">Get peace of mind overnight.</p>
                                        <Button onClick={onNavigateToPricing} variant="outline" size="sm" className="w-full max-w-[140px] h-9 text-xs border-blue-500/30 hover:bg-blue-500/10 text-blue-300">View Add-on</Button>
                                     </div>
                                 </div>
                             )}
                             <div className="w-full pt-6 border-t border-border/10 mt-auto">
