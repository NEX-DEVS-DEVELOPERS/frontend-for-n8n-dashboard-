...
const AdminSupportRequests: React.FC<AdminSupportRequestsProps> = ({ adminPassword }) => {
    ...
    const fetchRequests = async () => {
        try {
            ...
            const response = await fetch('http://localhost:3001/api/admin/support-requests', {
                headers: {
    ...
    const handleMarkComplete = async (requestId: string) => {
        try {
            ...
            const response = await fetch(`http://localhost:3001/api/admin/support-requests/${requestId}/complete`, {
                method: 'POST',
                headers: {
                    'x-admin-password': pwd
                }
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="bg-zinc-900/50 border-b border-cyan-900/20">
                                ...
                                <th className="px-6 py-4 text-xs font-semibold text-cyan-400 uppercase tracking-wider">Subject</th>
